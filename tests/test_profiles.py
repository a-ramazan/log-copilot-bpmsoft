...
"""
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            metrics = result.run_summary["key_metrics"]

            self.assertGreaterEqual(diagnostics["parse_quality"]["score"], 0.75)
            self.assertEqual("high", diagnostics["parse_quality"]["label"])
...
"""
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue(result.findings)
            self.assertTrue((run_dir / "findings.json").exists())
            self.assertFalse((run_dir / "traffic_summary.csv").exists())
            self.assertFalse((run_dir / "latency_report.md").exists())
